...
class EditNotesPage extends StatefulWidget {
  final int id;
  const EditNotesPage({Key? key, required this.id}) : super(key: key);
...
class _EditNotesPageState extends State<EditNotesPage> {
  ...
  void getData() async {
    var n = await Provider.of<NotesProvider>(context, listen: false).getNotesById(widget.id);
    if (n!= null) {
      setState(() {
        titleController.text = n.title!;
        descriptionController.text = n.description!;
        _pickedImage = n.attachment!;
        isReminder = n.reminder!.isNotEmpty;
        _reminder = n.reminder!;
        _interval = n.reminderInterval!;
      });
    }
  }
